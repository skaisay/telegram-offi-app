...
export const useArticles = create<ArticlesState>()(
  persist(
    (set, get) => ({
      ...
      toggleSaved: (id) =>
        set((state) => {
          const newSavedArticles = state.savedArticles.includes(id)
          ...
          return {
            ...
            readingStats: {
              ...state.readingStats,
              articlesRead: state.readingStats.articlesRead + (newSavedArticles.length > state.savedArticles.length ? 1 : 0),
            },
          };
        }),
      toggleDownloaded: (id) =>
      ...
      getSavedArticlesCount: () => get().savedArticles.length,
    }),
    {
